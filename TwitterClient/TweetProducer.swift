...
struct TweetProducer {
    ...
    /// This is the `database` of possible tweets. See https://twitter.com/KyloR3n
    static let tweets = [
    ...
    /**
     Generates a new set of tweets based on a given date. Each tweet's `createdDate` is 10 mins from the last one.
     
     - parameter date:     The date the tweet
     - parameter pageSize: The number of tweets to generate. The terminology `pageSize` is used to mimic a REST API.
     
     - returns: An array of `Tweet` objects.
     */
    static func tweetsSinceDate(date: NSDate, pageSize: Int = 10) -> [Tweet] {
        let calendar = NSCalendar.currentCalendar()
        var nextTweets = [Tweet]()
        var tweetDate = calendar.dateByAddingUnit(.Minute, value: 10, toDate: date, options: .MatchNextTime) ?? date
        for _ in 0..<pageSize {
            let randomIndex = arc4random_uniform(UInt32(tweets.count - 1))
            let tweet = Tweet()
            tweet.createdDate = tweetDate
            tweet.favouriteCount = Int(arc4random_uniform(50000))
            tweet.retweetCount = Int(arc4random_uniform(500))
            tweet.id = Int(tweetDate.timeIntervalSince1970)
            tweet.body = tweets[Int(randomIndex)]
            nextTweets.append(tweet)
